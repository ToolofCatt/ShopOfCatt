...
export function MailWorkspace() {
  const { token, loading: authLoading } = useAuth(), { t } = useI18n(), { priceUsdt } = usePrices(); const text = t.mail;
  ...
  return <div className="mail-workspace">
    <aside className="mail-service-panel"><div className="mail-section-title"><h2>{text.rentTitle}</h2><span>Gmail</span></div>
      ...
      {product && <div className="mail-purchase"><div className="mail-purchase-product"><ServiceIcon name={product.name} /><div><strong>{product.name}</strong><span>Gmail</span></div><b>{priceUsdt(Number(product.price)).primary}<small>{text.perMail}</small></b></div><p>{text.remaining}: {product.stock.toLocaleString()}</p>
        ...
        {!token && !authLoading ? <Link className="mail-primary" href="/login">{text.login}</Link> : <button className="mail-primary" disabled={!catalog?.purchaseEnabled || !validQuantity || !enough || busy || pendingPurchase} onClick={() => setConfirm('rent')}><span>{busy ? text.renting : text.rent}</span><strong>{priceUsdt(total).primary}</strong></button>}
    ...
    <section className="mail-history"><div className="mail-heading"><h1>{text.title}<span>{mails.length}</span></h1><button className="mail-button" onClick={() => void exportMail()} disabled={!token || !mails.length}><Download size={16} />{text.export}</button></div>
      ...
      <div className="mail-table" data-mail-scroll>{shown.map(m => <article key={m.id} className="mail-row"><div className="mail-address"><div><span>{m.email}</span><button className="mail-icon" aria-label={`${text.copyMail} ${m.email}`} onClick={() => void copy(m.email)}><Copy size={14} /></button></div><p>{m.service} <span>{priceUsdt(Number(m.price)).primary} {text.perMail}</span></p></div><div className="mail-row-codes">{m.codes[0] ? <><button className="mail-code latest" aria-label={`${text.copyCode} ${m.codes[0].code}`} onClick={() => void copy(m.codes[0].code)}><span>{m.codes[0].code}</span><Copy size={13} /></button><OldCodePopover mailbox={m} /></> : <span className="mail-waiting"><Clock3 size={13} />{text.noCodes}</span>}</div><div className="mail-status"><span>{m.closed ? <CircleSlash size={13} /> : m.codes.length ? <Check size={13} /> : <Clock3 size={13} />}{m.closed ? text.closed : m.codes[0] && Date.now() - new Date(m.codes[0].receivedAt).getTime() < 9000 ? text.newCode : m.codes.length ? text.received : text.waiting}</span>{!m.closed && m.canRead && <button onClick={() => setConfirm(m)}>{text.close}</button>}{m.pollFailed && <small>{text.readFailed}</small>}{!m.canRead && <small>{text.readUnsupported}</small>}</div></article>)}
    ...
    <dialog ref={dialog} className="mail-confirm" onCancel={e => { if (busy) e.preventDefault(); else setConfirm(null); }} onClose={() => setConfirm(null)}><h2>{confirm === 'rent' ? text.confirmTitle : text.closeTitle}</h2><p>{confirm === 'rent' ? text.confirmHint : text.closeHint}</p>{confirm === 'rent' && <div className="mail-confirm-total"><span>{product?.name} × {quantity}</span><strong>{priceUsdt(total).primary}</strong></div>}{error && <p role="alert">{error}</p>}<footer><button className="mail-button" disabled={busy} onClick={() => setConfirm(null)}>{text.cancel}</button><button className="mail-primary" disabled={busy} onClick={() => void (confirm === 'rent' ? submit() : stop())}>{busy ? text.loading : confirm === 'rent' ? text.rent : text.close}</button></footer></dialog>
  </div>;
}
