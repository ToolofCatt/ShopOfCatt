...
- `docker-compose.yml` — `postgres` (UTF-8 initdb, bound to `127.0.0.1`) → `api` (waits for db healthy) → `web` (waits for api healthy), plus `proxy` (Caddy, always on: single origin, automatic Let's Encrypt, `NEXT_PUBLIC_API_URL=/api`) and `backup` (periodic `pg_dump` into `./backups`). Web and API bind to `${APP_BIND:-127.0.0.1}` so the proxy is the only way in from the Internet. Config comes from a root `.env` created from `.env.docker.example`; `JWT_SECRET`, `POSTGRES_PASSWORD`, `ADMIN_EMAIL` and `ADMIN_PASSWORD` are mandatory (`${VAR:?...}`) and ship blank, and the API/seed additionally reject the placeholder values that appear in this repo.
